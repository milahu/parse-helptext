...
while [ ${#s[@]} != 0 ]; do a="${s[0]}"; s=("${s[@]:1}"); case "$a" in
  ...
  -a|--abr) g; __abr+=("$v"); A+=("$a" "$v"); continue;;
  -V|--tvbr) g; __tvbr+=("$v"); A+=("$a" "$v"); continue;;
  -v|--cvbr) g; __cvbr+=("$v"); A+=("$a" "$v"); continue;;
  -c|--cbr) g; __cbr+=("$v"); A+=("$a" "$v"); continue;;
  --he) : $((__he++)); A+=("$a"); continue;;
  -q|--quality) g; __quality+=("$v"); A+=("$a" "$v"); continue;;
  ...
  --native-resampler) g; __native_resampler+=("$v"); A+=("$a" "$v"); continue;;
...
if true; then
  ...
  [ "$__formats" != 0 ] && echo "__formats: $__formats"
  ...
  [ "$__he" != 0 ] && echo "__he: $__he"
  for i in "${!__quality[@]}"; do v="${__quality[$i]}"; echo "__quality $i: ${v@Q}"; done
  [ "$__adts" != 0 ] && echo "__adts: $__adts"
  [ "$__no_smart_padding" != 0 ] && echo "__no_smart_padding: $__no_smart_padding"
